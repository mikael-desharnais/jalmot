...
ModelEditorElementCM=function(title){
	...
	this.clicked=function(){
		...
		if (this.mode=="edit"){
			fetcher.setURL(new URL(Ressource.getConfiguration().getValue('AliasName')+'model_editor/'+this.modelEditor+'/',jQuery.parseJSON(this.descriptor.actionner.data('url-params'))));
		}else if (this.mode=="create") {
			fetcher.setURL(new URL(Ressource.getConfiguration().getValue('AliasName')+'model_editor/'+this.modelEditor+'/',{source:'create'}));
		}else {
			fetcher.setURL(new URL(Ressource.getConfiguration().getValue('AliasName')+'model_editor/'+this.modelEditor+'/delete/',jQuery.parseJSON(this.descriptor.actionner.data('url-params'))));
		}
		if (this.mode=="delete"){
...
SourceParamsModelEditorElementCM=function(title){
	...
	this.clicked=function(){
		...
		if (this.mode=="edit"){
			...
			fetcher.setURL(new URL(Ressource.getConfiguration().getValue('AliasName')+'model_editor/'+this.modelEditor+'/',jQuery.extend({source:create},this.descriptor.actionner.closest('.window_panel').data('object').url.params)));
		}else {
			fetcher.setURL(new URL(Ressource.getConfiguration().getValue('AliasName')+'model_editor/'+this.modelEditor+'/delete/',jQuery.parseJSON(this.descriptor.actionner.data('url-params'))));
		}
		if (this.mode=="delete"){
